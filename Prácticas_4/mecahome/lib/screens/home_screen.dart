...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void _alternarEstado(Dispositivo dispositivo) async {
    ...
    if (exito) {
      _cargarDispositivos(); // Recargamos la pantalla si se actualizó en Rails
    }
  }

  // Operación DELETE del CRUD
  void _eliminarDispositivo(int id) async {
    // Mostramos un diálogo de confirmación para que no se borre por error
    ...
    if (confirmar == true) {
      ...
      if (exito) {
        _cargarDispositivos(); // Recargamos el dashboard
  ...
  void _mostrarFormularioCrear() {
    ...
    showModalBottomSheet(
      context: context,
      isScrollControlled: true, // Permite que el teclado no tape el formulario
      builder: (context) => StatefulBuilder( // Permite actualizar los dropdowns dentro del modal
              ...
              ElevatedButton(
                ...
                onPressed: () async {
                  if (nombreController.text.isNotEmpty) {
                    // Preparamos el mapa de datos simulando los valores por defecto del backend
                    Map<String, dynamic> nuevoDispositivo = {
                      'nombre': nombreController.text,
                      'tipo': tipoSeleccionado,
                      'marca': marcaSeleccionada,
                      'estado': 'off', // Empieza apagado por defecto
                    };

                    // Si es clima, le damos valores iniciales para que el Strategy funcione
  ...
  Future<Dispositivo?> _cambiarTemperaturaDeseada(Dispositivo disp, double cambio) async {
    ...
    if (exito) {
      // 1. Forzamos el refresco de la interfaz principal y la petición a Rails
      _cargarDispositivos(); 
      
      // 2. Buscamos el dispositivo en nuestra lista local para extraer el dato limpio
      try {
        ...
        builder: (context, setModalState) => Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              // Fila para controlar la temperatura
             // Fila para controlar la temperatura con refresco en tiempo real
              Row(
                ...
                children: [
                  IconButton(
                    ...
                    onPressed: () async {
                      // Usamos setModalState para que el modal se redibuje con el nuevo dato
              ...
              SegmentedButton<String>(
                ...
                onSelectionChanged: (Set<String> nuevoModo) {
                  ...
                  Navigator.pop(context); // Cierra el panel al cambiar
                },
              ),
              const SizedBox(height: 20),
            ],
        ...
        builder: (context, snapshot) {
          ...
          return GridView.builder(
            ...
            gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(
              crossAxisCount: 2, // Dos columnas tipo dashboard
                      ...
                      Row(
                        ...
                        children: [
                          Icon(
                            ...
                            dispositivo.tipo == 'iluminacion' ? Icons.lightbulb : Icons.power,
                            color: isOn ? Colors.indigo : Colors.grey,
                            size: 32,
                          ),
                          // Cambiamos el texto plano por un Row con la marca y la papelera
                          Row(
                            children: [
                              Text(
                              ...
                              IconButton(
                                ...
                                constraints: const BoxConstraints(), // Quita el margen interno por defecto
